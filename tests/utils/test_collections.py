...
def maketree():
    ...
    def recurmake(node, depth, width):
        for i in range(1, width + 1):
            current = collections.TreeNode('value_{:02d}'.format(i))
            if depth > 0:
                recurmake(current, depth - 1, width)
            node.append(current)
        return node

    def builder(depth, width) -> collections.TreeNode:
        root = collections.TreeNode('root')
...
def makenode():
    ...
    def builder(value: str, *children):
        node = collections.TreeNode(value)
        for child in children or []:
            node.append(collections.TreeNode(child))
        return node
...
class TestTreeNode:
    ...
    def test_remove_expect_removed_child(self, makenode):
        parent, child = makenode('parent', 'child'), makenode('child')
        with mock.patch('booktag.utils.collections.TreeNode.set_parent') as mm:
            removed = parent.remove(child)
            mm.assert_called_once_with(None)
    ...
    def test_sort_where_sort_by_supported_key_function(self, maketree):
        depth, degree = 2, 2
        root = maketree(depth, degree)
        key = mock.Mock(return_value=1)
        root.sort(key=key)
        assert key.call_count == depth * (degree + 1)
